...
if (s.includes(OLD_MUTE)) {
  if (s.split(OLD_MUTE).length - 1 !== 1) { console.log('ERR ancre muted ('+(s.split(OLD_MUTE).length-1)+')'); process.exit(1); }
...
if (s.includes(OLD_MIME)) {
  s = s.replace(OLD_MIME, `  // On privilegie les formats qui declarent AUSSI une piste audio
  const cands = [
