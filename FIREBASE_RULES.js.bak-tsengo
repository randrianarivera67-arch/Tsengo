    ...
    match /posts/{postId} {
      ...
      allow update: if request.auth != null &&
        (
          ...
          ||
          // Tous : réactions, commentaires ary compteur de vues ihany
          // ✅ FIX : 'views' nampiana — taloha ny increment(views) dia LAVINA mangina
          // ho an'ny post an'olon-kafa, ka ny batch manontolo dia tsy nandeha.
          request.resource.data.diff(resource.data).affectedKeys().hasOnly(['reactions', 'comments', 'views'])
        );
      allow delete: if request.auth != null && (isAdmin() || resource.data.uid == request.auth.uid);
    }
