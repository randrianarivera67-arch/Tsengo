  ...
  async function acceptRequest(req) {
    ...
    try {
      ...
      await addDoc(collection(db, 'notifications'), {
        ...
        fromPhoto: userProfile.photoURL || '',
        type: 'friendAccepted',
        message: `${userProfile.fullName} ${t('accept').toLowerCase()} ny fangatahana namanao`,
        read: false,
        createdAt: serverTimestamp(),
      });
            ...
            return (
              <div key={user.uid} className="card" style={{ padding: '12px 14px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 12 }}>
                <img
                  ...
                  alt="" className="avatar" style={{ width: 50, height: 50 }}
                  onClick={() => navigate(`/profile/${user.uid}`)} style={{ cursor: 'pointer', width: 50, height: 50, borderRadius: '50%', objectFit: 'cover' }}
                />
                <div style={{ flex: 1 }}>
