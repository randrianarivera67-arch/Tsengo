...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // ✅ FIX : tsy writeBatch atomika intsony — ny batch taloha dia LAVINA manontolo
    // raha vao nisy post iray tsy azo novaina (règles), ka tsy nisy vue voaisa mihitsy.
    // Isaky ny document : ny iray tsy mandeha dia tsy manimba ny hafa.
  ...
  // Suggestions d'amis — LAHARAM-PAHAMEHANA : amis en commun betsaka indrindra,
  // avy eo ny mpampiasa vao nisoratra anarana, avy eo ny mitovy localisation.
  useEffect(() => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // FIL D'ACTUALITÉS v2 — "ordre gelé" (façon Facebook), tsy misy bug fihetsiketsehana
  //  • Pagination cursor 20/20 hatramin'ny infini (getDocs + startAfter)
  //  • Ny FILAHARANA dia kajiana rehefa (chargement / refresh / page vaovao) IHANY
  //    → tsy mifindra ny cartes rehefa mijery ianao (namaha ny bug "long press
  //    → compte hafa" izay vokatry ny re-tri temps réel teo ambany rantsan-tanana)
  //  • Ny temps réel dia manavao ny votoaty (réactions, commentaires, vues) eo
  //    amin'ny toerany fotsiny — TSY manova filaharana mihitsy
  //  • Publication vaovao avy amin'olon-kafa → bouton "N nouvelles publications"
  //    (toy ny Facebook) ; ny publication-nao manokana dia mipoitra eo ambony avy hatrany
  ...
  // Mpampiasa vao nisoratra anarana (30 farany) — ny publication-dry zareo dia
  // omena vahana kely amin'ny "récent" araka ny localisation misy azy
  ...
  useEffect(() => {
    scoreCtxRef.current = {
      ...
      aff: new Set([...(userProfile?.friends || []), ...(userProfile?.following || [])]),
      myCity: (userProfile?.currentCity || userProfile?.hometown || '').trim().toLowerCase(),
      lat: viewerLoc?.lat, lng: viewerLoc?.lng,
      seed: shuffleSeed,
      newUids: newUserUids || new Set(),
    };
  }, [currentUser, userProfile?.friends, userProfile?.following, userProfile?.currentCity, userProfile?.hometown, viewerLoc?.lat, viewerLoc?.lng, shuffleSeed, newUserUids]);

  function rankPosts(rows) {
    const ctx = scoreCtxRef.current;
    const nowMs = Date.now(); const nowD = new Date();
    const rnd = (id) => {
      let h = 2166136261; const str = String(id) + ':' + ctx.seed;
  ...
  useEffect(() => {
    const blocked = userProfile?.blocked || [];
    const myFriends = userProfile?.friends || [];
    const byId = new Map(feedRaw.map(x => [x.id, x]));
    const visible = (p) => !!p && !blocked.includes(p.uid)
      && (p.uid === currentUser?.uid || (p.audience === 'friends' ? myFriends.includes(p.uid) : p.audience !== 'me'));
    setPosts(order.map(id => byId.get(id)).filter(visible));
    setReelPosts(feedRaw.filter(p => visible(p) && p.mediaType === 'video' && p.mediaURL));
...
const fields = {
      ...
      authorCity: userProfile.currentCity || userProfile.hometown || '',   // localisation (classement)
      isDecree,
  ...
  return (
    <div style={{ padding:0 }}>
      ...
      {pendingNew.length > 0 && (
        <button
          onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); refreshFeed(); }}
          style={{ position:'fixed', top:66, left:'50%', transform:'translateX(-50%)', zIndex:85,
            background:'linear-gradient(135deg,#1B84FF,#1877F2)', color:'white', border:'none',
            borderRadius:22, padding:'9px 18px', fontWeight:700, fontSize:13, cursor:'pointer',
            boxShadow:'0 6px 20px rgba(24,119,242,.45)', fontFamily:'Poppins',
            display:'flex', alignItems:'center', gap:7, animation:'fadeSlide .3s ease' }}>
          <HiArrowUp size={15}/> {pendingNew.length} nouvelle{pendingNew.length>1?'s':''} publication{pendingNew.length>1?'s':''}
        </button>
      )}
      {boostTarget && (
        ...
        return (
          <div style={{ position:'fixed', inset:0, background:'#000', zIndex:300, display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, padding:'6px 12px' }}>
              <img src={avatarSrc(storyViewer.group.photo)}
            ...
            <div style={{ flex:1, position:'relative', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', background: cur.mediaType === 'text' ? (cur.bgColor || '#1877F2') : 'transparent' }}>
              {cur.mediaType === 'video'
                ...
                : cur.mediaType === 'text'
                ? <p style={{ color: cur.textColor || 'white', fontSize: cur.fontSize || 30, fontWeight:800, textAlign: cur.align || 'center', padding:'0 30px', wordBreak:'break-word', whiteSpace:'pre-wrap', ...(cur.textPos ? { position:'absolute', left:(cur.textPos.x*100)+'%', top:(cur.textPos.y*100)+'%', transform:'translate(-50%,-50%)', width:'86%', padding:0 } : null) }}>{cur.text}</p>
                ...
                <div onClick={e => e.stopPropagation()} style={{ background:'white', borderRadius:'20px 20px 0 0', padding:18, width:'100%', maxWidth:480, maxHeight:'60vh', overflowY:'auto' }}>
                  ...
                  {storyReactors.map(r => (
                    <div key={r.uid} onClick={() => { setStoryReactors(null); setStoryViewer(null); navigate(`/profile/${r.uid}`); }}
                      ...
                      <img src={avatarSrc(r.photo)} alt="" style={{ width:38, height:38, borderRadius:'50%', objectFit:'cover' }} />
      ...
      <div className="card post-card" style={{ padding:12, marginBottom:8 }}>
        <div style={{ display:'flex', gap:10, alignItems:'center' }}>
          <img src={avatarSrc(userProfile?.photoURL)} alt="" className="avatar" style={{ width:40, height:40, flexShrink:0 }}/>
          ...
          <div className="card post-card animate-fade" style={{ marginBottom:14, border:boosted?'1px solid #a855f755':undefined, position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10, flex:1, minWidth:0 }}>
                  <div style={{ position:'relative', flexShrink:0, width:44, height:44 }}>
                    ...
                    <img onClick={() => navigate(`/profile/${post.uid}`)}
                      src={avatarSrc(post.authorPhoto)}
                  ...
                  {postMenu === post.id && (
                    <div style={{ position:'absolute', top:'100%', right:0, background:'white', border:'1px solid #E4E6EB', borderRadius:14, boxShadow:'0 6px 24px rgba(0,0,0,.16)', minWidth:220, zIndex:50, overflow:'hidden' }}>
                      {isOwn && <>
                        ...
                        {!post.groupId && !post.sharedFrom && <button onClick={() => { setBoostTarget({ type:'post', id: post.id, ownerUid: post.uid, title: post.content?.slice(0,60) || 'Votre publication', thumbnailURL: post.mediaURL || '' }); setPostMenu(null); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'13px 18px', background:'none', border:'none', cursor:'pointer', color:'#050505', fontSize:14.5, fontWeight:400, borderBottom:'1px solid #F0F2F5', fontFamily:'Poppins' }}><HiLightningBolt size={17} color="#a855f7"/> Booster</button>}
                      </>}
            ...
            {(() => {
              ...
              return (
                ...
                {post.mediaURLs?.length > 1 ? (
                  ...
                  <div style={{ marginTop:8, marginLeft:-16, marginRight:-16 }}>
                    {post.isMusic ? <MusicPostCard post={post} height={140}/> : post.mediaType==='image' ? <SmartImage className="media-fit" src={post.mediaURL || post.thumbURL} onClick={e=>{e.stopPropagation();openPost(post.id);}} style={{ width:'100%', borderRadius:0, maxHeight:'100%', objectFit:'contain', display:'block', cursor:'zoom-in' }}/> : <div className="media-fit"><FeedVideo /* media-fit voafono */ src={post.mediaURL} poster={post.thumbURL} dataSaver={dataSaver || lite} onOpenReels={()=>navigate('/reels',{state:{startId:post.id}})} style={{ width:'100%', borderRadius:0, maxHeight:'100%', objectFit:'contain', display:'block', background:'#000' }} /></div>}
                  </div>
                )}
            ...
            <div /* CRISTAL_TEXTE */ className={(post.sharedFrom || post.eventFrom || !(post.mediaURL || (post.mediaURLs && post.mediaURLs.length))) ? 'media-cristal' : undefined}
              ...
              {post.content && (
                <p
                  onClick={e => {
                    // Raha vao vita appui long (copie), tsy mandeha amin'ny post
                    if (textPressedRef.current) { textPressedRef.current = false; e.stopPropagation(); return; }
                  }}
              ...
              <div style={{ padding:'0 16px 14px', borderTop:'1px solid #E4E6EB' }}>
                {post.comments?.map(c => {
                  ...
                  <div key={c.id} style={{ display:'flex', gap:8, marginTop:10 }}>
                    <img src={avatarSrc(c.authorPhoto)} alt="" className="avatar" style={{ width:32, height:32, flexShrink:0, cursor:'pointer' }} onClick={() => navigate(`/profile/${c.uid}`)}/>
                    <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ flex:1, position:'relative', minWidth:0 }}>
                    <input className="input" placeholder={replyTo[post.id]?`Répondre à ${replyTo[post.id]}...`:t('writeComment')} value={cmtText[post.id]||''}
                      onChange={e=>{
                        ...
                        const mAt = v.match(/@([\p{L}0-9_-]*)$/u);
                        if (mAt) { loadMentionPeople(); setMentionQuery({ postId: post.id, q: mAt[1].toLowerCase() }); }
                        else setMentionQuery(null);
                      }}
                      ...
                      return (
                        <div key={g.uid} className="story-card" onClick={() => openStories(g)} style={{ width:92, height:150 }}>
                          {last.mediaType === 'video'
                            ...
                            : <img src={last.mediaURL} alt="" />}
                          <div className="story-gradient" />
                          <img className="story-avatar" src={avatarSrc(g.photo)} alt="" />
                          <span className="story-name">{g.uid === currentUser.uid ? 'Votre story' : g.name?.split(' ')[0]}</span>
                        </div>
                      );
                    })}
