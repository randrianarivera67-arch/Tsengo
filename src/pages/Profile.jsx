...
import {
  ...
  HiReply, HiPhone, HiLocationMarker, HiLink, HiBookmark
} from 'react-icons/hi';
...
export default function Profile() {
  ...
  const [postReportTarget, setPostReportTarget] = useState(null);  // post à signaler
  const [tagModalPost,   setTagModalPost] = useState(null);        // post à identifier
  ...
  // ── Actions au niveau PUBLICATION (menu identique au fil d'actualités) ──
  ...
  async function togglePostBlock(post) {
    ...
    const msg = already ? `Débloquer ${post.authorName || profile.fullName} ?` : `Bloquer ${post.authorName || profile.fullName} ? Vous ne verrez plus ses publications.`;
    if (!window.confirm(msg)) return;
    try {
  ...
  async function submitPostReport(motif, detail) {
    ...
    try {
      await addDoc(collection(db, 'reports'), {
        type: 'post', targetId: post.id, targetUid: post.uid, targetAuthor: post.authorName || profile.fullName,
        reportedBy: currentUser.uid, reportedByName: userProfile.fullName,
  ...
  async function openPostTagModal(post) {
    setTagModalPost(post);
    const init = {}; (post.taggedUids || []).forEach(u => { init[u] = true; });
    setTagSelected(init);
    const myFriends = userProfile?.friends || [];
    const list = await Promise.all(myFriends.map(fuid =>
      getDoc(doc(db, 'users', fuid)).then(sn => sn.exists() ? { uid: fuid, ...sn.data() } : null).catch(() => null)
    ));
            ...
            <div style={{ position:'relative' }} onClick={e=>e.stopPropagation()}>
              ...
              {postMenu===post.id && (
                <div style={{ position:'absolute', top:'100%', right:0, background:'white', border:'1px solid #E4E6EB', borderRadius:14, boxShadow:'0 6px 24px rgba(0,0,0,.16)', minWidth:220, zIndex:50, overflow:'hidden' }}>
                  {isOwnPost && <>
                    <button onClick={() => { setEditPost(post); setEditContent(post.content); setPostMenu(null); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'13px 18px', background:'none', border:'none', cursor:'pointer', color:'#050505', fontSize:14.5, fontFamily:'Poppins', borderBottom:'1px solid #F0F2F5' }}><HiPencil size={17} color="#1877F2"/> Modifier</button>
                    {!post.groupId && !post.sharedFrom && <button onClick={() => { setBoostTarget({ type:'post', id: post.id, ownerUid: post.uid, title: (post.content||'').slice(0,60) || 'Votre publication', thumbnailURL: post.mediaURL || '' }); setPostMenu(null); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'13px 18px', background:'none', border:'none', cursor:'pointer', color:'#050505', fontSize:14.5, fontFamily:'Poppins', borderBottom:'1px solid #F0F2F5' }}><HiLightningBolt size={17} color="#a855f7"/> Booster</button>}
                  </>}
                  <button onClick={() => { togglePostSave(post.id); setPostMenu(null); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'13px 18px', background:'none', border:'none', cursor:'pointer', color:'#050505', fontSize:14.5, fontFamily:'Poppins', borderBottom:'1px solid #F0F2F5' }}><HiBookmark size={17} color="#F2B300"/> {(userProfile?.saved||[]).includes(post.id) ? 'Retirer' : 'Enregistrer'}</button>
                  {isOwnPost && (
                    <button onClick={() => { openPostTagModal(post); setPostMenu(null); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'13px 18px', background:'none', border:'none', cursor:'pointer', color:'#050505', fontSize:14.5, fontFamily:'Poppins', borderBottom:'1px solid #F0F2F5' }}><HiUserAdd size={17} color="#1877F2"/> Identifier</button>
                  )}
                  {post.mediaURL && <button onClick={() => { downloadMedia(post.mediaURL, post.mediaType); setPostMenu(null); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'13px 18px', background:'none', border:'none', cursor:'pointer', color:'#050505', fontSize:14.5, fontFamily:'Poppins', borderBottom: isOwnPost ? 'none' : '1px solid #F0F2F5' }}><HiDownload size={17} color="#3b82f6"/> Télécharger</button>}
                  {isOwnPost && (
                    <button onClick={() => { deletePost(post.id); setPostMenu(null); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'13px 18px', background:'none', border:'none', cursor:'pointer', color:'#FF2D8D', fontSize:14.5, fontFamily:'Poppins', borderTop:'1px solid #F0F2F5' }}><HiTrash size={17}/> Supprimer</button>
                  )}
                  {!isOwnPost && <>
                    <button onClick={() => { setPostReportTarget(post); setPostMenu(null); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'13px 18px', background:'none', border:'none', cursor:'pointer', color:'#050505', fontSize:14.5, fontFamily:'Poppins', borderBottom:'1px solid #F0F2F5' }}><HiFlag size={17} color="#F2B300"/> Signaler</button>
                    <button onClick={() => { togglePostBlock(post); setPostMenu(null); }} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, padding:'13px 18px', background:'none', border:'none', cursor:'pointer', color:'#FF2D8D', fontSize:14.5, fontFamily:'Poppins' }}><HiBan size={17}/> {(userProfile?.blocked||[]).includes(post.uid) ? 'Débloquer' : 'Bloquer'}</button>
                  </>}
                </div>
              )}
            </div>
      ...
      <div style={{ padding:12 }}>
        ...
        {activeTab==='videos'&&(videoPosts.length===0
          ...
          : <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:8 }}>
              {videoPosts.map(p => (
                <VideoThumb key={p.id} src={p.mediaURL} poster={p.thumbURL}
                  onClick={() => navigate(`/post/${p.id}`)}
                  style={{ aspectRatio:'9/16', width:'100%', borderRadius:10 }} playSize={40}/>
              ))}
            </div>
        )}
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:8 }}>
              {storyArchive.map(st => (
                <div key={st.id} style={{ position:'relative', borderRadius:12, overflow:'hidden', aspectRatio:'9/16', background:'#000' }}>
                  {st.mediaType === 'video'
                    ? <VideoThumb src={st.mediaURL} poster={st.thumbURL} style={{ width:'100%', height:'100%', borderRadius:12 }} playSize={34}/>
                    : <SmartImage src={st.mediaURL} minH={120} style={{ width:'100%', height:'100%', objectFit:'cover', borderRadius:12 }}/>}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background:'white', borderRadius:'20px 20px 0 0', padding:18, width:'100%', maxWidth:520, maxHeight:'85vh', overflowY:'auto' }}>
            ...
            {souvenirs.map(sp => (
              <div key={sp.id} onClick={() => { setSouvenirs(null); navigate(`/post/${sp.id}`); }}
                ...
                {sp.mediaURL
                  ? (sp.mediaType === 'video'
                      ? <VideoThumb src={sp.mediaURL} poster={sp.thumbURL} style={{ width:56, height:56, borderRadius:10, flexShrink:0 }} playSize={22} minH={56}/>
      ...
      {tagModalPost && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:400, display:'flex', alignItems:'flex-end', justifyContent:'center' }} onClick={() => setTagModalPost(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background:'white', borderRadius:'20px 20px 0 0', padding:20, width:'100%', maxWidth:480, maxHeight:'80vh', overflowY:'auto' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
              <h3 style={{ fontWeight:800, color:'#1877F2', fontSize:16 }}>Identifier des amis</h3>
              <button onClick={() => setTagModalPost(null)} style={{ background:'none', border:'none', cursor:'pointer', color:'#65676B' }}><HiX size={20}/></button>
            </div>
            {tagFriendsList.length === 0 && <p style={{ fontSize:13, color:'#65676B', textAlign:'center', padding:'16px 0' }}>Aucun ami à identifier.</p>}
            ...
            <button onClick={savePostTags} className="btn-blue" style={{ width:'100%', marginTop:14, padding:'10px 0', borderRadius:10, fontSize:14 }}>Enregistrer</button>
          </div>
        </div>
      )}
    </div>
