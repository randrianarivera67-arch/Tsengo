...
export default function JejoStudio({ currentUser, userProfile, onClose, onPublished }) {
  ...
  useEffect(() => {
    ...
    // mangina IHANY raha tsy tafiditra ao anaty video ny hira
          ...
          <Sheet title="Embellir (beauté)" onClose={() => setPanel(null)}>
            ...
            {beautyOn && !bLoading && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <Slider label="Lisser la peau" value={bSmooth} onChange={setBSmooth} icon={FxIc.smooth(16)} />
                ...
                <div>
                  <p style={{ color: 'rgba(255,255,255,.75)', fontSize: 12.5, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
...
// Pictogrammes des effets de beaute (SVG)
const FxIc = {
  smooth: (sz = 16) => (
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
        <span style={{ color: 'rgba(255,255,255,.75)', fontSize: 12.5, display: 'flex', alignItems: 'center', gap: 6 }}>
